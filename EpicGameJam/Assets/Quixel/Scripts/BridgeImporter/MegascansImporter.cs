...
namespace Quixel
{
    ...
    public class MegascansImporter : Editor
    {
        ...
        private Newtonsoft.Json.Linq.JObject mapNames;
        ...
        public string ImportMegascansAssets(Newtonsoft.Json.Linq.JObject objectList)
        {
            ...
            //Map name overrides.
            mapNames = (Newtonsoft.Json.Linq.JObject)objectList["mapNameOverride"];
...
#region Mesh Processing Methods
        ...
        void ProcessMeshes(Newtonsoft.Json.Linq.JArray meshComps)
        {
            ...
            string meshName = meshPath + "/" + finalName.Replace("$mapName", "").Replace("$resolution", "");
            ...
            for (int i = 0; i < meshComps.Count; ++i)
            {
                ...
                newPath = newPath.Contains("$variation") ? newPath.Replace("$variation", "VAR" + i.ToString()) : newPath;
        ...
        void CreatePrefabs(bool hasBillboard = false)
        {
            ...
            string prefabName = finalName.Replace("$mapName", "").Replace("$resolution", "").Replace("$lod", "");
            ...
            for (int i = 0; i < variants.Count; ++i)
            {
                ...
                string tempPrefabName = prefabName.Contains("$variation") ? prefabName.Replace("$variation", "VAR" + i.ToString()) : prefabName + "_Var" + i.ToString();
                string finalName = prefabPath + "/" + tempPrefabName + ".prefab";
                finalName = finalName.Replace("(Clone)", "");
                UnityEngine.Object pf = AssetDatabase.LoadAssetAtPath(finalName, typeof(UnityEngine.Object));
                if (!pf)
                {
#if UNITY_2019
                    PrefabUtility.SaveAsPrefabAsset(g, finalName);
#else
...
#pragma warning disable
        ...
        void CreatePrefabs()
        {
            ...
            for (int i = 0; i < transforms[0].Count; ++i)
            {
                ...
                go.isStatic = true;
                string tempPrefabName = prefabName.Contains("$variation") ? prefabName.Replace("$variation", "VAR" + i.ToString()) : prefabName;
                string finalName = prefabPath + "/" + tempPrefabName + ".prefab";
                finalName = finalName.Replace("(Clone)", "");
                UnityEngine.Object pf = AssetDatabase.LoadAssetAtPath(finalName, typeof(UnityEngine.Object));
                if (!pf)
                {
#if UNITY_2019
                    PrefabUtility.SaveAsPrefabAsset(go, finalName);
#else
...
#region Formatting Utilities
        ...
        string ConstructPath(Newtonsoft.Json.Linq.JObject objectList) {
            ...
            defPath = ValidateFolderCreate(defPath, finalFolderName);
            return defPath;
        }
